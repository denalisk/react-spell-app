...
import IPreSpell1 from "../models/prespell-one.interface";
import ISpellMapper from "../models/spell-mapper.interface";

export class PreSpell1Mapper implements ISpellMapper<IPreSpell1> {
    constructor(private sourceName: string) {

    }
    
    public map(preSpell: IPreSpell1): Spell {
        const newSpell = {
            id: 0,
            name: preSpell.Name,
            description: preSpell.Description,
            page: this.sourceName,
    ...
    private mapComponent = (componentString: string): string[] => {
        ...
        if (components.length === 2) {
            ...
            components[1] = components[1].charAt(0).toUpperCase() + components[1].slice(1);
        }
